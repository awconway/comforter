...
OUTPUT_DIR = Path("/Users/ac/comforter/artifacts/autogluon_metwithin48h")
TARGET = "METWithin48h"
EXCLUDE = ["ID", "Mort30d", "ICUWithin48h", "DeathHospDisch"]
SEED = 42
...
def _plot_calibration(y_true: np.ndarray, prob: np.ndarray, out_path: Path) -> None:
    ...
    ax.set_title("AutoGluon METWithin48h Test Calibration Curve (10 quantile bins)")
    ax.set_xlim(0, 1)
    ax.set_ylim(0, 1)
...
def main() -> None:
    ...
    # CPU-friendly non-deep models only.
    hyperparameters = {
        "CAT": {},
        "RF": {},
        "XT": {},
    ...
    preds = pd.DataFrame(
        {
            ...
            "proba_metwithin48h": prob_pos,
            "proba_no_metwithin48h": 1.0 - prob_pos,
        }
    )
