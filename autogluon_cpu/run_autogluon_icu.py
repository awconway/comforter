...
OUTPUT_DIR = Path("/Users/ac/comforter/artifacts/autogluon_icuwithin48h")
TARGET = "ICUWithin48h"
EXCLUDE = ["Id", "DeathHospDisch", "Mort30d", "METWithin48h"]
SEED = 42
...
def _plot_calibration(y_true: np.ndarray, prob: np.ndarray, out_path: Path) -> None:
    ...
    ax.set_title("AutoGluon ICUWithin48h Test Calibration Curve (10 quantile bins)")
    ax.set_xlim(0, 1)
    ax.set_ylim(0, 1)
    ...
    preds = pd.DataFrame(
        {
            ...
            "proba_icuwithin48h": prob_pos,
            "proba_no_icuwithin48h": 1.0 - prob_pos,
        }
    )
